...
namespace PicSimulator.Simulator
{
    public class Simulation
    {
        ...
        public void selectTypeOfCommand(int cmd)
        {
            ...
            bool numberOfCycles; // to check, if the instruction is 1 or 2 Cycles
            // now we can check which type the cmd is

            if (data == 0) // Byte-oriented operations
            {   // 0b00_xxxx_xxxx_xxxx
                numberOfCycles = selectCommand00(cmd);

            }
            else if (data == 1)  // Bit-orienented operations
            {   // 0b01_xxxx_xxxx_xxxx
                numberOfCycles = selectCommand01(cmd);
            }
            else if (data == 2)   // control operations (Goto, call)
            {   // 0b10_xxxx_xxxx_xxxx
                numberOfCycles = selectCommand02(cmd);
            }
            else if (data == 3)  // literal operations
            {   // 0b11_xxxx_xxxx_xxxx
                numberOfCycles = selectCommand03(cmd);
            }
            else
            {
                throw new Exception("Instruction Error!");
            }
            SystemOperations.UpdateRuntime(numberOfCycles);
